                    ...
                    <div class="card">
                        <div class="header">
                            ...
                            <ul class="header-dropdown m-r--5">
                                <a href="{{route('packages.create')}}" class="btn btn-success waves-effect">
                                ...
                                <table class="table table-bordered table-striped table-hover dataTable js-exportable">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>{{$row->date}}</td>
                                            <td>
                                        <a href="{{ route('packages.edit',$row->id) }}" class="btn btn-info waves-effect">
                                                    <i class="material-icons">edit</i>
                                                </a>
        
                                             <button class="btn btn-danger waves-effect" type="button" onclick="deletePackages({{ $row->id }})">
                                                    <i class="material-icons">delete</i>
                                                </button>
                                                <form id="delete-form-{{ $row->id }}" action="{{ route('packages.destroy',$row->id) }}" method="POST" style="display: none;">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
                                            </td>
